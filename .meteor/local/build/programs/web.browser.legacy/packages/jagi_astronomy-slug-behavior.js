...
Behavior.create({
  ...
  createClassDefinition: createClassDefinition,
  apply: apply,
...
var throwParseError = Module.get('core').utils.throwParseError;
...
function createClassDefinition() {
  ...
  var definition = {
    fields: (_fields = {}, _fields[this.options.slugFieldName] = {
      type: String,
...
function generateSlug(doc) {
  ...
  var Class = doc.constructor; // Check validity of the class for generating slug.
  ...
  var oldSlug = doc.get(this.options.slugFieldName); // If the current slug is not empty and we can not update, then we have to
  // stop here.
  ...
  var newSlug = value.toLowerCase(); // Remove diacriticts.
  ...
  if (newSlug !== oldSlug) {
    ...
    if (this.options.unique) {
      ...
      var selector = (_selector = {}, _selector[this.options.slugFieldName] = newSlug, _selector);
      var count = Class.find(selector, {
        disableEvents: true
      }).count();
      ...
      if (count > 0) {
        ...
        var prefix = newSlug + this.options.separator;
        var re = new RegExp('^' + prefix + '\\d+$');
        selector[this.options.slugFieldName] = re; // Limit the amount of fields being fetched to only the slug field.

        var options = {
          fields: (_fields = {}, _fields[this.options.slugFieldName] = 1, _fields),
          disableEvents: true
        }; // Set the first number that will be added at the end of the slug to 2.

        var index = 2; // Loop through all the documents with the same slug.

        Class.find(selector, options).forEach(function (d) {
          var dSlug = d.get(_this.options.slugFieldName);
...
function beforeSave(e) {
  var doc = e.currentTarget;
  var Class = doc.constructor; // Check if a field from which we want to create a slug has been
  // modified.
