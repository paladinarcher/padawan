...
if (Meteor.isServer) {
  describe('Questions', function () {
    ...
    // todo unaswerall
    it('unaswerAll makes allAnsweredUsers and answeredQuestions return an empty string', function () {
      let q = FactoryBoy.create('question1');
      let u = FactoryBoy.create('user2');
      let a = FactoryBoy.build('answer1');
      //u.MyProfile.UserType.answerQuestion(q);
      ...
      let u2 = User.find({ _id: u._id }).fetch()
      //console.log(u2[0].MyProfile.UserType.AnsweredQuestions[0].QuestionID);
      chai.assert.strictEqual(u2[0].MyProfile.UserType.AnsweredQuestions[0].QuestionID, q._id, "Answer ID is not the same as Question ID");
      let ansUser = q.allAnsweredUsers();
      // console.log(ansUser.fetch()[0].MyProfile.UserType);
      chai.assert.strictEqual(ansUser.fetch()[0]._id, u._id, "question.allAnsweredUsers doesn't match the user ID");
      let ansUser1 = q.allAnsweredUsers().fetch();
      // console.log('ansUser1: ', ansUser1);
      q.unanswerAll(false);
      let ansUser2 = q.allAnsweredUsers().fetch();
      // console.log('ansUser2: ', ansUser2);
      chai.assert.strictEqual(ansUser2.length, 0, 'allAnsweredUsers should be empty');
      //console.log('ansUser2: ', ansUser2[0].MyProfile.UserType);
      let uAfterUnanswer = User.find({ _id: u._id }).fetch();
      chai.assert.strictEqual(uAfterUnanswer[0].MyProfile.UserType.AnsweredQuestions.length, 0, 'allAnsweredUsers should be empty');
      // console.log('uAfterUnanswer: ', uAfterUnanswer);
      // console.log('uAfterUnanswer: ', uAfterUnanswer[0].MyProfile.UserType.AnsweredQuestions);
    });
