...
if (Meteor.isServer) {
  describe('Questions', function() {
    ...
    it('allAnsweredUsers returns at least one user that answered the question', function() {
      ...
      User.update(
        { _id: u._id },
        { $push: { 'MyProfile.UserType.AnsweredQuestions': a } }
      );
      myStub.restore();
      ...
      chai.assert.equal(
        u2[0].MyProfile.UserType.AnsweredQuestions[0].QuestionID,
        q._id,
        'Answer ID is not the same as Question ID'
      );
      ...
      chai.assert.equal(
        ansUser.fetch()[0]._id,
        u._id,
        "question.allAnsweredUsers doesn't match the user ID"
      );
    });
    it('reset() resets TimesAnswered and SumOfAnswers', function() {
      ...
      chai.assert(
        taStart == 1 && soaStart == -2,
        'Error while adding an answer'
      );
      q.reset();
      let taFinish = q.TimesAnswered.LeftSum;
      let soaFinish = q.SumOfAnswers.LeftSum;
      chai.assert(
        taFinish == 0 && soaFinish == 0,
        'Error while reseting answers'
      );
    });
  });
}
