...
const MyersBriggsBit = Class.create({
  ...
  helpers: {
    addValue(value) {
      ...
      this.QuestionCount++;
      this.Value =
        this.QuestionCount == 0 ? 0 : this.Totals / this.QuestionCount;
    },
    removeValue(value) {
      this.QuestionCount--;
    ...
    unAnswerQuestion(answer, skipSlice) {
      ...
      if (!skipSlice) {
        if (index == 0) {
          ...
          this.AnsweredQuestions = this.AnsweredQuestions.slice(
            0,
            index
          ).concat(this.AnsweredQuestions.slice(index + 1));
        }
      }
      answer.unanswer();
      _.each(answer.Categories, function(cat) {
      ...
      console.log(
        'User Answer Count: ' + before + ' => ' + this.AnsweredQuestions.length
      );
    },
    getQnaire(qnid) {
      thisQn = {};
  ...
  meteorMethods: {
    addQnaireResponse(newRespId) {
      ...
      if (!respExists) {
        ...
        Meteor.users.update(
          { _id: userId },
          { $push: { 'MyProfile.QnaireResponses': newRespId } }
        );
      }
    ...
    removeQnaireResponse(respId) {
      ...
      if (respExists) {
        ...
        Meteor.users.update(
          { _id: userId },
          { $pull: { 'MyProfile.QnaireResponses': respId } }
        );
      }
    }
  }
