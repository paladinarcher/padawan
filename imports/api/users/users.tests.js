...
FactoryBoy.define('nonAdminUser1', User, {
  ...
  MyProfile: {
    ...
    technicalSkillsData: 'th3TestK3y'
  },
  teams: [],
  roles: {},
  profile: {
...
if (Meteor.isServer) {
  describe('User', function() {
    ...
    it('user full name is returned by full name method', function returnUserFullName(done) {
      ...
      chai.assert(
        result === true,
        'user full name is not being returned by full name method'
      );
      done();
    });
    ...
    it('user can update their profile information', function testUpdateProfile(done) {
      resetDatabase();
      let nonAdminUser = FactoryBoy.create('nonAdminUser1', {
        _id: '1234567899912839'
      });
      ...
      let result =
        loggedInUser.MyProfile.firstName === 'Charlie' &&
        loggedInUser.MyProfile.lastName === 'Testerino'
          ? true
          : false;
    ...
    it('user can change name with changeName method', function testChangeNameMethod(done) {
      ...
      result =
        firstName === changedFirstName && lastName === changedLastName
          ? true
          : false;
      chai.assert(
        result === true,
        'User name is not changing using changeName method'
      );
      done();
    });
    ...
    it('user can add roles', function testAddRoleMethod(done) {
      ...
      chai.assert(
        result === true,
        'User role was not added via addRole method'
      );
      stub.restore();
      done();
    });
    ...
    it('user can remove roles', function testRemoveRoleMethod(done) {
      ...
      result =
        user.roles.__global_roles__.indexOf(newRole) === -1 ? true : false;

      chai.assert(
        result === true,
        'User role was not removed via removeRole method'
      );
      stub.restore();
      done();
    });
