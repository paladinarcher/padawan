...
Meteor.startup(() => {
    ...
    if (addSamples == 1) {
        ...
        // create default questions to load on startup
        ...
        if(Question.find().count() < 1) {
          for (let question in DefaultQuestions) {
            let q = new Question({
              CreatedBy: theAdmin._id,
              Category: DefaultQuestions[question].Category,
              Text: DefaultQuestions[question].Text,
              Categories: DefaultQuestions[question].Categories,
              LeftText: DefaultQuestions[question].LeftText,
              RightText: DefaultQuestions[question].RightText,
              Active: DefaultQuestions[question].Active
            })
            q.save();
          }
        }
